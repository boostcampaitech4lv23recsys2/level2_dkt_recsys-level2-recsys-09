...
class DataToSequence(Dataset):
    def __init__(self, groups, seq_len):
        ...
        for user_id in groups.index:
            item_id, test_id, time_lag, elaps_time, answer_code=groups[user_id]
            ...
            if len(item_id) > self.seq_len:
                ...
                if initial > 2:
                    ...
                    self.samples[f"{user_id}_0"] = (
                        ...
                        elaps_time[:initial], answer_code[:initial]
                    )
                # 시퀀스만큼 몇번 진행되어야 하는지
                ...
                for c in range(chunks):
                    ...
                    self.samples[f"{user_id}_{c+1}"] = (
                        ...
                        elaps_time[start:end], answer_code[start:end]
                    )
            # 주어진 시퀀스 이하면 그냥 그대로 저장
            else:
                self.user_ids.append(f"{user_id}")
                self.samples[f"{user_id}"] = (item_id,test_id,time_lag,elaps_time,answer_code)
    ...
    def __getitem__(self, index):
        ...
        # 시퀀스별로 잘라진 애들에 대해
        item_id, test_id, time_lag, elaps_time, answer_code = self.samples[user_id]
        seq_len = len(item_id)
        ...
        return itemid ,timelag, elapstime, answercode, label
        ...
        for user_id in groups.index:
            ...
            if len(item_id) > self.seq_len:
                ...
                self.samples[f"{user_id}"] = (
                    ...
                    elaps_time[-seq_len:], answer_code[-seq_len:])
            else:
                self.user_ids.append(f"{user_id}")
                self.samples[f"{user_id}"] = (item_id,test_id,time_lag,elaps_time,answer_code)
